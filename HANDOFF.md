...
The suite is **207 pass, 0 fail**. ChaCha20 and Poly1305 are complete and every
...
1. **SHA-256, then HKDF-SHA-256.** SHA-256 needs 32-bit rotate-right and a
   64-word message schedule; `rotl32` already exists and rotate-right by `n` is
   rotate-left by `32-n`. Watch the cost: `ADD8`'s cost is the *product* of its
   operands, and SHA-256 is roughly five hundred 32-bit adds per block, so a
   cheaper carry-detecting kernel may be worth building first.

2. **PQC is explicitly out of scope.** Keccak and NTT are a later mountain.
3. `CONVENTIONS.md` predates every tool in `tools/`. It does not mention
...
4. **Tier 6, differential fuzz, is still declared and still absent.** It is
