...
| 5 declared contracts | Does the code obey the interface composition depends on? | A routine states where the pointer must be and which cells must be clear, as `; ASSERT ptr=N` and `; ASSERT zero A:B` lines. They are comments, so a plain interpreter ignores them and portability is untouched; `BFI_CONTRACTS=1 tools/bfi` checks them EVERY time execution reaches that point, including on every pass through a loop. This turns an interface from a comment that might lie into a checked fact, and it is the tier that catches the bugs the output tiers cannot see: a pointer left off its anchor, scratch that was not clean on entry, a routine entered at the wrong offset. Every one of those cost hours during the build before this existed. Self-tested: the checker must catch a wrong pointer and a dirty scratch cell, and must stay inert without the flag. |
...
| 3a style consistency | Is the style the same across every source file? | `tools/bfstyle` reads the sources as data and enforces one canonical style: annotations on their own line at column 0 (never trailing on a code line), a title then an `IO` section then a `TAPE MAP`, and no run of more than 12 code lines without an annotation. This tier exists because style silently drifted: the hand-written primitives used trailing annotations while the assembled files put them above the code, and **no other tier could see it**, because every other tier checks what the code *computes*, not how it *reads*. Self-tested by feeding it each divergence. The assemblers all emit through one shared `tools/bfemit`, so they cannot drift from each other by construction. |
| 3b size budget | Is this file short enough that a person would actually read it? | `tools/bfstyle` fails any `.bf` over 2000 lines. This is not a style nicety: the project exists to commit brainfuck a human can follow, so **size is part of legibility**. The rule was added after the composites drifted to 42k lines (the block function), 66k (the stream cipher) and, in an AEAD that was never committed, 903k. Every one of those passed all the other checks, which is exactly the point � they inspect form, not whether reading is possible. The cause was always the same: brainfuck has no indexed addressing, so a loop over data at varying positions is hard, and unrolling is easy. Unrolling multiplies size. `index/fetch8.bf` and `index/store8.bf` are the hand-written idiom that makes loops possible instead. |
