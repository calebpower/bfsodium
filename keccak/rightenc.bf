; bfsodium RIGHTENC : NIST SP 800_185 section 2 3 1's right_encode
;
; HAND WRITTEN skeleton;
;
; INTERFACE entry=3 exit=12 footprint=0:17
; IO  in:  x{4} LE                      (4 bytes)
;     out: the encoding  its count byte LAST
;
; right_encode of x is the shortest big endian run of bytes that holds x  with a
; count of them after; the count is at least one  so nought
...
[
  -
<<<<<<<<                                                       ; x's bytes  most significant first
  [->>>>>>>>>>>>>+<<<<<<<<<<<<<]
>>>>>>>>>>>>>>                                                 ; and the count after them
  +
